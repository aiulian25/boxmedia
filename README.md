...
docker run --rm --entrypoint /usr/bin/python3.11 ghcr.io/aiulian25/boxmedia:1.0.3 \
...
docker run --rm -v "$PWD/secrets:/secrets" --entrypoint /usr/bin/python3.11 \
  ghcr.io/aiulian25/boxmedia:1.0.3 -m app.core.crypto genkey /secrets/boxmedia-new.key
docker run --rm -v "$PWD/secrets:/secrets" -v "$PWD/data:/data" \
  --entrypoint /usr/bin/python3.11 ghcr.io/aiulian25/boxmedia:1.0.3 \
